    ...
    public model_functions={
        ...
        addResident:async(req,name,flat_no,society_name,area,unit,next)=>{
            return new Promise(async(resolve,reject)=>{
                ...
                this.connection.query(sql,[society_name],async(err,result)=>{
                    if(err)throw err;
                    var sql1="SELECT resident_id FROM resident_details WHERE owner_name=?"
                    this.connection.query(sql1,[name],async(err,result1)=>{
                        if(err)throw err;
                        if(result1.length>0)
                        {
                            ...
                            this.connection.query(SQL,async(err,result1)=>{
                                if(err)throw err;
                ...
                try{    
                    ...
                    this.connection.query(SQL,[society_name],async(err,result)=>{
                        ...
                        if(result.length>0)
                        {
                            ...
                            this.connection.query(sql,[result[0].society_id],async(err,result1)=>{
                                if(err)throw err;
                                if(result1.length>0)
                                {
                ...
                try{
                    ...
                    this.connection.query(SQL,[society_name],async(err,result)=>{
                        ...
                        if(result.length){
                            ...
                            this.connection.query(SQL1,[id],async(err,result1)=>{
                                if(err)throw err;
                                if(result1.length>0)
                                {
                                    var data={status:"true",message:result1};
        ...
        createBill:async(req,resident,service,water,sink,repair,other,from,to,due,next)=>{
            return new Promise(async(resolve,reject)=>{
                try{
                    ...
                    var result=await Master_functions1.sqlProcess(SQL,this.connection,next);
                    if(result.affectedRows>0)
                    {
                        var bill_id=result.insertId;
                        var SQL2=`SELECT resident_id FROM resident_details WHERE owner_name='${resident}'`;
                        var result2=await Master_functions1.sqlProcess(SQL2,this.connection,next);
                        console.log(result2);
                        if(result2.length>0)
                        {
                            ...
                            var result3=await Master_functions1.sqlProcess(SQL3,this.connection,next);
                            if(result3[0].amount_pending==null)
                            {
                                ...
                                var result4=await Master_functions1.sqlProcess(SQL4,this.connection,next)
                                ...
                                var result4=await Master_functions1.sqlProcess(SQL5,this.connection,next);
                                if(result4.affectedRows>0)
                                {
                                    resolve({status:true})
                                }
            ...
            return new Promise(async(resolve,reject)=>{
                try{
                    ...
                    if(result.length>0)
                    {
                        ...
                        var result1=await Master_functions1.sqlProcess(SQL1,this.connection,next);
                        var result2=await Master_functions1.sqlProcess(SQL2,this.connection,next);
                        ...
                        var result5=await Master_functions1.sqlProcess(SQL5,this.connection,next);
